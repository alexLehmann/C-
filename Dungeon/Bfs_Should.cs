...
namespace Dungeon
{
	...
	public class Bfs_Should
	{
		...
		private void AreValidPaths(List<Point>[] paths, Map map, int[] expectedLengths)
		{
			...
			{
				IsValidPath(map, paths[i], expectedLengths[i]);
			}
			Assert.AreEqual(expectedPathCount, uniqueEndpoints.Length, "Each path must lead to unique chest");
		}

		private void IsValidPath(Map map, List<Point> path, int expectedLength)
		{
